...
* than a message in log file(if possible)/stderr is written and the error_number field in structure is filled with one of the
* "error" value
*
* @param[in, out] stk - pointer to stack
*
...
int stack_ok(Stack *stk){
    ...
    if(!stk->log_file){
        stk->error_number = LOG_FILE_PTR_ERR;
    ...
    if(stk->canary1 != stack_canary1){
        stk->error_number = CANARY_1_DEAD;
        return 1;
    }

    if(stk->canary2 != stack_canary2){
        stk->error_number = CANARY_2_DEAD;
        return 1;
    }
#endif

    if(!stk->data){
        stk->error_number = INVALID_DATA_PTR_ERR;
        return 1;
    }

    if(stk->size < 0){                    ///как ошибку:поменять проверки size и data_canary
        stk->error_number = INVALID_SIZE_ERR;
        return 1;
    }

    if(stk->capacity < 0){
        stk->error_number = INVALID_CAPACITY_ERR;
        return 1;
    }
    ...
    if(*(long int*)(stk->data + stk->capacity * sizeof(data_t) + sizeof(stack_data_canary)) != stack_data_canary ||
       *(long int*)(stk->data) != stack_data_canary){
        stk->error_number = DATA_CANARY_DEAD;
        return 1;
    }
#endif

    if(!stk->name){
        stk->error_number = INVALID_NAME_ERR;
        return 1;
    }

    if(stk->error_number < 0){
        stk->error_number = INVALID_ERROR_ERR;
        return 1;
    }

#ifdef HASH_P
    if(stk->hash != stack_hash(stk)){
        stk->error_number = INCORRECT_HASH_ERR;
        return 1;
    }
#endif
...
int stack_pop(Stack *stk, data_t *value){
    ...
    if(!value){
        stk->error_number = POP_ERR;
        ASSERT_OK(stk);
        return 1;
    }

    if(!stk->size){
        stk->error_number = UNDERFLOW_ERR;
        ASSERT_OK(stk);
        return 1;
    }
...
Stack *stack_create(const char *log_file, const char *stack_name){
    ...
    stk->error_number = INIT;

    fprintf(stk->log_file, "\terror_number = %d\n", stk->error_number);
...
void stack_dump(Stack *stk){
    ...
    fprintf(file, 	"\t\t\t\t\t STACK_DUMP:\n"
                     ...
                     "\t size = %d\n" "\t capacity = %d\n"
                     "\t data[%d elem.] = [%p]\n",
            stk->name, stk, stk->error_number? "ERROR!!!" : "ok",
#ifdef CAN_P
            stk->canary1,
#endif
    ...
    fprintf(stk->log_file, "\t error_number = %s\n", stack_print_error(stk->error_number));
...
const char *stack_print_error(enum error error_number){
    ...
    switch(error_number){
        ...
        case INIT:
            return "INIT: error_number has initial value\n";

        default:
            return "ERROR: error_number has impossible value\n";
    }
}
...
//	printf("stk = %p\n" "&stk->canary = %p\n" "&stk->name = %p\n" "&stk->size = %p\n" "&stk->capacity = %p\n"
//			"&stk->log_file = %p\n" "&stk->data = %p\n" "&stk->error_number = %p\n" "&stk->hash = %p\n" "&stk->canary2 = %p\n",
//			stk, &stk->canary1, &stk->name, &stk->size, &stk->capacity, &stk->log_file, &stk->data, &stk->error_number,
